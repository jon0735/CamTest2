...
public class MainScript : MonoBehaviour
{
    ...
    void Update()
    {
        ...
        if (Input.GetKeyDown(KeyCode.A)){
            ...
            foreach(Vector3 dir in this.directions){
                Util.createSphere(dir, UnityEngine.Color.white, scale: 0.03f);
            }
        }

        if (Input.GetKeyDown(KeyCode.S)){
            this.directions = Util.fibSphereSample(n:sphereSampleSize);
            foreach(Vector3 dir in this.directions){
                // GameObject g = GameObject.CreatePrimitive(PrimitiveType.Sphere);
                Util.createSphere(dir, UnityEngine.Color.black);
    ...
    private void testAdjustDistance(){
        ...
        Util.createSquare(objCentre, UnityEngine.Color.black, scale: 0.1f);
        float score;
